...
presentation:
	cd presentation && $(PR_TEX) $(PR_TEXFLAGS) $(PR_FILE).tex
	@if [ -n "$(PR_BIB)" ]; then cd presentation && $(PR_BIB) $(PR_FILE).aux; fi
	cd presentation && $(PR_TEX) $(PR_TEXFLAGS) $(PR_FILE).tex
	cd presentation && $(PR_TEX) $(PR_TEXFLAGS) $(PR_FILE).tex
...
clean:
	...
	cd presentation && rm -f $(CLEAN_EXTS)
